...
namespace CVZ_Core
{
    ...
    public abstract class IConvergenceZone : ISerializable
    {
	   ...
	   public FromCodeModality hierarchicalModality;
        ...
        public void CreateHierarchicalModality(int size, float influence, bool isSynchrone)
        {
		  ...
		  hierarchicalModality = new FromCodeModality(name, "hierarchical", size);
        ...
        public void Step(float enactionFactor)
	   {
		  ...
		  if (errorLog != null)
		  {
			 if (starLoggingTime == -1.0)
				starLoggingTime = currentTime = System.DateTime.Now.TimeOfDay.TotalMilliseconds;
			 else
				currentTime = System.DateTime.Now.TimeOfDay.TotalMilliseconds; ;
